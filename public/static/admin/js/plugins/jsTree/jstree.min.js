(function(factory) {
	...
	$.jstree.core.prototype = {
		...
		get_node: function(obj, as_dom) {
			...
			try {
				if(this._model.data[obj]) {
					...
					if(typeof obj === "string" && this._model.data[obj.replace(/^#/, "")]) {
						obj = this._model.data[obj.replace(/^#/, "")]
					} else {
						if(typeof obj === "string" && (dom = $("#" + obj.replace($.jstree.idregex, "\\$&"), this.element)).length && this._model.data[dom.closest(".jstree-node").attr("id")]) {
							obj = this._model.data[dom.closest(".jstree-node").attr("id")]
						} else {
							if((dom = $(obj, this.element)).length && this._model.data[dom.closest(".jstree-node").attr("id")]) {
								obj = this._model.data[dom.closest(".jstree-node").attr("id")]
							} else {
								if((dom = $(obj, this.element)).length && dom.hasClass("jstree")) {
			...
			var w = null,
				...
				func = function(data, undefined) {
					...
					var dat = data.dat,
						...
						p = m[par],
						sel = data.sel,
						tmp, i, j, rslt, parse_flat = function(d, p, ps) {
							if(!ps) {
								ps = []
							} else {
								ps = ps.concat()
		...
		set_icon: function(obj, icon) {
			...
			if(icon === false) {
				...
				if(icon === true || icon === null || icon === undefined || icon === "") {
			...
			if(this.settings.checkbox.cascade.indexOf("up") !== -1 || this.settings.checkbox.cascade.indexOf("down") !== -1) {
				this.element.on("model.jstree", $.proxy(function(e, data) {
					...
					for(i = 0, j = this._data[t ? "core" : "checkbox"].selected.length; i < j; i++) {
						if((s.indexOf("down") === -1 || $.inArray(this._data[t ? "core" : "checkbox"].selected[i], obj.children_d) === -1) && (s.indexOf("up") === -1 || $.inArray(this._data[t ? "core" : "checkbox"].selected[i], obj.parents) === -1)) {
	...
	$.jstree.plugins.dnd = function(options, parent) {
		this.bind = function() {
			parent.bind.call(this);
			this.element.on("mousedown.jstree touchstart.jstree", this.settings.dnd.large_drag_target ? ".jstree-node" : ".jstree-anchor", $.proxy(function(e) {
				...
				if(obj && obj.id && obj.id !== $.jstree.root && (e.which === 1 || e.type === "touchstart") && (this.settings.dnd.is_draggable === true || ($.isFunction(this.settings.dnd.is_draggable) && this.settings.dnd.is_draggable.call(this, (mlt > 1 ? this.get_top_selected(true) : [obj]), e)))) {
					...
					}, '<div id="jstree-dnd" class="jstree-' + this.get_theme() + " jstree-" + this.get_theme() + "-" + this.get_theme_variant() + " " + (this.settings.core.themes.responsive ? " jstree-dnd-responsive" : "") + '"><i class="jstree-icon jstree-er"></i>' + txt + '<ins class="jstree-copy" style="display:none;">+</ins></div>')
				}
			}, this))
		}
	};
	$(function() {
		...
		$(document).on("dnd_start.vakata.jstree", function(e, data) {
			...
			if(ins && ins._data && ins._data.dnd) {
				...
				data.helper.children().attr("class", "jstree-" + ins.get_theme() + " jstree-" + ins.get_theme() + "-" + ins.get_theme_variant() + " " + (ins.settings.core.themes.responsive ? " jstree-dnd-responsive" : "")).find(".jstree-copy").first()[data.data.origin && (data.data.origin.settings.dnd.always_copy || (data.data.origin.settings.dnd.copy && (data.event.metaKey || data.event.ctrlKey))) ? "show" : "hide"]();
	...
	$.jstree.plugins.massload = function(options, parent) {
		...
		this._load_node = function(obj, callback) {
			...
			if(d) {
				return this[typeof d === "string" ? "_append_html_data" : "_append_json_data"](obj, typeof d === "string" ? $($.parseHTML(d)).filter(function() {
					return this.nodeType !== 3
				}) : d, function(status) {
					callback.call(this, status);
	...
	(function($) {
		$.vakata.search = function(pattern, txt, options) {
			...
			search = function(text) {
				...
				var i, j, textLen = text.length,
					scoreThreshold = MATCH_THRESHOLD,
					bestLoc = text.indexOf(pattern, MATCH_LOCATION),
					binMin, binMid, binMax = patternLen + textLen,
					lastRd, start, finish, rd, charMatch, score = 1,
					locations = [];
				if(bestLoc !== -1) {
